...
def sidecar_migration_fingerprint(conn: sqlite3.Connection) -> str:
    """SHA-256 over the DDL of exactly the objects ``migrate_v49`` installs.

    The receipt's whole-database ``schema_sha256`` depends on the *source*
    database (a fresh ``schema.sql`` install and a decade-old migrated corpus
    hash differently), so it cannot be recomputed by a fixture test -- which
    is how a stale receipt shipped in B3 round 0: the committed hash was
    captured before ``migrate_v49`` gained its six ``replica_content_*``
    triggers, and nothing deterministic compared it against HEAD.

    This fingerprint is source-independent: it hashes only the sidecar's own
    schema objects (every ``context_concept*`` table/index/trigger, the two
    ``context_citations_bound_*`` guard triggers, and the six
    ``replica_content_context_concept*`` replication triggers), whose DDL is
    byte-identical however the database reached v49. FTS5 shadow tables
    (``context_concept_fts_%``) are excluded: their DDL is generated by the
    SQLite library from the virtual-table declaration (which *is* hashed), so
    including them would tie the receipt to the library version without
    detecting any migration change. ``test_migrations.py`` proves this
    selection covers the complete v49 delta and that the retained receipt
    matches a fresh install of the shipped migration.
    """
    rows = conn.execute(
        """
...
def capture_sidecar_receipt(
    ...
    *,
    ...
    return {
        ...
        # Source-independent hash of the migration's own objects; pinned
        # against a fresh install by test_migrations.py so a receipt captured
        # from an older migrate_v49 can no longer ship unnoticed (B3 review
        # round 1, Important #1). evidence_version 1 -> 2 for this field.
        "sidecar_objects_sha256": sidecar_migration_fingerprint(conn),
