"""Concept-sidecar provisioning: the SessionWeaver schema lifted unchanged (Phase B3).

The schema DEFINITIONS below (``_PAYLOAD_OBJECTS``/``_METADATA_OBJECTS`` and the
version constants) are lifted byte-for-byte from SessionWeaver's
``session_weaver/concept_schema.py``, which was written for exactly this lift: it
records and verifies its own schema identity independently of ``PRAGMA user_version``,
which this package continues to own. Any edit to a definition changes
``SCHEMA_FINGERPRINT`` and is caught both by the pinned-fingerprint test here and by
SessionWeaver's ``verify_installed_schema`` at read time — treat the definitions as
frozen and coordinate changes across both repositories.

``ensure_concept_sidecar`` installs the sidecar on a fully migrated store (upstream
schema v47 with the context layer present), adopts an exact pre-existing payload, or
verifies an already-installed sidecar. It never changes ``user_version``.
"""
...
class SidecarError(RuntimeError):
    """The sidecar is drifted, tampered, or its invariants cannot be satisfied."""
...
def _verify_objects(conn: sqlite3.Connection) -> None:
    ...
    if not complete or not exact:
        raise SidecarError("Concept schema is incomplete or has fingerprint drift")
...
def _verify_clock(conn: sqlite3.Connection) -> None:
    ...
    if (
        ...
        raise SidecarError("Concept clock counters are invalid")


def verify_installed_sidecar(conn: sqlite3.Connection) -> None:
    ...
    if marker is None or tuple(marker) != (SCHEMA_VERSION, SCHEMA_FINGERPRINT):
        raise SidecarError("Concept schema version/fingerprint mismatch")
    _verify_clock(conn)
...
def ensure_concept_sidecar(conn: sqlite3.Connection) -> str:
    """Install, exactly adopt, or verify the sidecar without changing ``user_version``.

    Returns ``"installed"``, ``"adopted"``, ``"verified"`` or
    ``"skipped: context layer not installed"``. Raises :class:`SidecarError` on
    drift, tampering, or unsatisfiable invariants — never silently repairs.
    """
    if conn.execute("PRAGMA foreign_keys").fetchone()[0] != 1:
        raise SidecarError("Concept schema requires foreign_keys=ON")
    upstream = conn.execute("PRAGMA user_version").fetchone()[0]
    if upstream != UPSTREAM_SCHEMA_VERSION:
        raise SidecarError(
            f"Unsupported upstream schema v{upstream}; expected v{UPSTREAM_SCHEMA_VERSION}"
    ...
    if not _context_layer_present(conn):
        return "skipped: context layer not installed"
    with _atomic(conn):
        metadata_present = (
            conn.execute(
        ...
        if instance is None:
            raise SidecarError("Pinned context instance identity is missing")
        conn.execute(
