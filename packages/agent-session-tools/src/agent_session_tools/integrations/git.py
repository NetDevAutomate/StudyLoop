...
def get_git_context(project_path: str | Path) -> GitContext | None:
    ...
    try:
        ...
        result = subprocess.run(
            ["git", "rev-parse", "--git-dir"], cwd=path, capture_output=True, text=True, timeout=2
        )
        ...
        # Get current commit
        result = subprocess.run(
            ["git", "rev-parse", "HEAD"], cwd=path, capture_output=True, text=True, timeout=2
        )
        if result.returncode == 0:
        ...
        result = subprocess.run(
            ["git", "status", "--porcelain"], cwd=path, capture_output=True, text=True, timeout=2
        )
        if result.returncode == 0:
            lines = [line for line in result.stdout.strip().split("\n") if line.strip()]
            context.uncommitted_files = [line[3:] for line in lines]  # Remove status prefix
            context.is_dirty = bool(context.uncommitted_files)

        # Get diff statistics
        if context.is_dirty:
            result = subprocess.run(
                ["git", "diff", "--stat"], cwd=path, capture_output=True, text=True, timeout=2
            )
            if result.returncode == 0:
...
def format_git_context_for_export(git_context: GitContext) -> str:
    ...
    if git_context.commit_hash and git_context.commit_message:
        lines.append(f"**Commit:** `{git_context.commit_hash}` - {git_context.commit_message}")
