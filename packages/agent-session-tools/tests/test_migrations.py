...
class TestMigrationV49ConceptSidecar:
    ...
    def test_sidecar_migration_fingerprint_covers_exactly_the_v49_delta(self, tmp_path):
        """The receipt fingerprint's object selection is the full v49 delta.

        ``sidecar_migration_fingerprint`` selects objects by name; this proves
        the selection equals everything ``migrate_v49`` actually creates (the
        FTS5 shadow tables aside -- their DDL is generated by the SQLite
        library from the fingerprinted virtual-table declaration), so a future
        edit to the migration cannot slip an object past the receipt
        regression below.
        """
        import hashlib
        ...
        try:
            ...
            before = {(row[0], row[1]) for row in conn.execute(ddl).fetchall()}

            _description, migrate_v49 = MIGRATIONS[49]
            migrate_v49(conn)
            set_user_version(conn, 49)
            conn.commit()

            delta = [
                row
                for row in conn.execute(ddl).fetchall()
                if (row[0], row[1]) not in before
            ]
            delta_names = {name for _kind, name, _sql in delta}

            # The exact objects whose absence made round 0's receipt stale.
            assert {
                f"replica_content_{table}_{event}"
                for table in ("context_concepts", "context_concept_events")
                for event in ("insert", "update", "delete")
            } <= delta_names

            hashed = [
                row for row in delta if not row[1].startswith("context_concept_fts_")
            ]
            expected = hashlib.sha256(
                "\n".join(f"{kind}:{name}:{sql}" for kind, name, sql in hashed).encode(
                    "utf-8"
                )
            ).hexdigest()
            assert sidecar_migration_fingerprint(conn) == expected
        finally:
            conn.close()

    def test_retained_receipt_matches_the_shipped_migration(self, fresh_db):
        """The committed live receipt must describe the migration at HEAD.

        B3 review round 1, Important #1: the round-0 receipt was captured at
        commit 1, before ``migrate_v49`` gained its six ``replica_content_*``
        triggers, and nothing deterministic caught the staleness. The whole-
        database ``schema_sha256`` cannot be recomputed here (it depends on
        the source corpus), so the receipt carries a source-independent
        ``sidecar_objects_sha256`` that a fresh install of the shipped
        ...
        assert receipt["evidence_version"] >= 2, (
            "receipt predates the sidecar_objects_sha256 field -- regenerate it "
        ...
        assert receipt["sidecar_objects_sha256"] == sidecar_migration_fingerprint(
            ...
            "committed receipt is stale: its sidecar DDL hash does not match a "
            "fresh install of the shipped migrate_v49 -- regenerate it with: "
            "pytest packages/agent-session-tools/tests/"
            "test_concept_sidecar_live.py -m live_concepts"
        )
