"""Tests for studyloop CLI commands using Click's CliRunner.
...
class TestTopics:
    def test_topics_with_monkeypatched_list(
        ...
        import studyloop.cli._sync as sync_mod
...
class TestReview:
    ...
    def test_review_nothing_due(self, runner: CliRunner, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        import studyloop.cli._review as review_mod
...
class TestWins:
    def test_wins_no_progress_data(
        ...
        monkeypatch.setattr(
            "studyloop.history.get_progress_summary",
            lambda: {},
        )
...
class TestProgress:
    def test_progress_records_successfully(
        ...
        monkeypatch.setattr("studyloop.history.record_progress", lambda *a, **kw: True)
    ...
    def test_progress_failure(self, runner: CliRunner, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("studyloop.history.record_progress", lambda *a, **kw: False)
...
class TestStreaks:
    def test_streaks_no_sessions(self, runner: CliRunner, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            "studyloop.history.get_study_streaks",
    ...
    def test_streaks_with_data(self, runner: CliRunner, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("studyloop.history.get_energy_session_data", lambda days: [])
        result = runner.invoke(cli, ["streaks"])
        assert result.exit_code == 0
...
class TestResume:
    def test_resume_no_sessions(self, runner: CliRunner, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("studyloop.history.get_last_session_summary", lambda: None)
    ...
    def test_resume_with_session(self, runner: CliRunner, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            "studyloop.history.get_last_session_summary",
            lambda: {
                "source": "claude_code",
