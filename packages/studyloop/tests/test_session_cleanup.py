...
from studyloop.session_state import ParkingEntry, TopicEntry
...
class TestPersistSessionData:
    ...
    def _call(self, study_id, state, topic_entries, notes, auto_persist=True):
        from studyloop.session.cleanup import _persist_session_data
    ...
    def test_auto_persist_skipped_when_disabled(self):
        ...
        with (
            patch("studyloop.history.end_study_session"),
            patch("studyloop.services.backlog.auto_persist_struggled", mock_backlog),
        ):
    ...
    def test_backlog_failure_does_not_abort_db_end(self):
        ...
        with (
            patch("studyloop.history.end_study_session", mock_end),
            patch(
                "studyloop.services.backlog.auto_persist_struggled",
                side_effect=RuntimeError("boom"),
            ),
        ):
    ...
    def test_flashcard_generation_called_with_slug(self):
        ...
        with (
            patch("studyloop.history.end_study_session"),
            patch("studyloop.services.flashcard_writer.write_session_flashcards", mock_writer),
            patch("studyloop.settings.load_settings", return_value=mock_settings),
            patch("studyloop.services.backlog.auto_persist_struggled"),
        ):
    ...
    def test_flashcard_failure_does_not_abort_db_end(self):
        ...
        with (
            patch("studyloop.history.end_study_session", mock_end),
            patch(
                "studyloop.services.flashcard_writer.write_session_flashcards",
                side_effect=RuntimeError("boom"),
            ),
            patch("studyloop.settings.load_settings"),
            patch("studyloop.services.backlog.auto_persist_struggled"),
        ):
    ...
    def test_db_failure_is_logged_not_raised(self):
        ...
        with (
            patch(
                "studyloop.history.end_study_session",
...
class TestSignalDashboardEnded:
    def test_writes_mode_ended(self):
        ...
        with patch("studyloop.session_state.write_session_state", mock_write):
...
class TestTeardownAgent:
    def test_calls_adapter_teardown_with_session_dir(self, tmp_path):
        ...
        with patch("studyloop.agent_launcher.AGENTS", {"kiro": mock_adapter}):
    ...
    def test_skips_when_no_adapter_found(self):
        ...
        with patch("studyloop.agent_launcher.AGENTS", {}):
            from studyloop.session.cleanup import _teardown_agent
...
class TestKillBackgroundProcesses:
    ...
    def test_sends_sigterm_to_matching_web_pid(self):
        ...
        state = {"web_pid": 9999}
        mock_run = MagicMock()
        mock_run.return_value.stdout = "python -m studyloop.cli web"
        mock_kill = MagicMock()
    ...
    def test_port_fallback_called_for_web(self):
        ...
        mock_kill_port.assert_any_call(8567, expected_cmd="studyloop")
...
class TestEndSessionCommon:
    ...
    def test_returns_topic_on_success(self):
        ...
        with (
            patch("studyloop.session_state.parse_topics_file", return_value=[]),
            patch("studyloop.session_state.parse_parking_file", return_value=[]),
            patch("studyloop.session.cleanup._persist_session_data"),
            patch("studyloop.session.cleanup._signal_dashboard_ended"),
            patch("studyloop.session.cleanup._teardown_agent"),
            patch("studyloop.session.cleanup._kill_background_processes"),
            patch("studyloop.session.cleanup._cleanup_tmux_and_files"),
        ):
    ...
    def test_all_helpers_called(self):
        ...
        with (
            patch("studyloop.session_state.parse_topics_file", return_value=[]),
            patch("studyloop.session_state.parse_parking_file", return_value=[]),
            patch("studyloop.session.cleanup._persist_session_data", persist_mock),
            patch("studyloop.session.cleanup._signal_dashboard_ended", signal_mock),
            patch("studyloop.session.cleanup._teardown_agent", teardown_mock),
            patch("studyloop.session.cleanup._kill_background_processes", kill_mock),
            patch("studyloop.session.cleanup._cleanup_tmux_and_files", cleanup_mock),
        ):
