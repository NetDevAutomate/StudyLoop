"""Integration tests for studyloop study — real tmux sessions with a mock agent.
...
def _studyloop(*args: str, env_overrides: dict | None = None, timeout: int = 10):
    ...
    env.pop("TMUX", None)
    return subprocess.run(
        ["uv", "run", "--project", str(PROJECT_DIR), "studyloop", *args],
        capture_output=True,
        text=True,
        timeout=timeout,
        env=env,
...
def _make_wrapper_agent(tmp_path: Path) -> str:
    ...
    script.write_text(
        textwrap.dedent("""\
        #!/bin/bash
        # Wrapper agent: uses the studyloop wrapper from the session dir
        # (same as what Claude Code does in a real session)
        sleep 2
        studyloop topic "Wrapper Test" --status learning --note "via wrapper"
        sleep 1
        studyloop park "Does the wrapper actually work?"
        # Wait for exit
        trap 'exit 0' INT TERM
        while true; do sleep 1; done
    """)
...
class TestSessionCreation:
    ...
    def test_session_directory_created(self, tmp_path):
        ...
        assert (session_dir / "studyloop").exists()  # wrapper script
...
class TestCleanup:
    ...
    def test_explicit_end_command(self, tmp_path):
        """studyloop study --end cleans up session."""
        agent = _make_mock_agent(tmp_path)
        info = _start_session(agent)
        session_name = info["session_name"]
...
class TestWrapperScript:
    ...
    def test_wrapper_script_exists_and_executable(self, tmp_path):
        ...
        wrapper = Path(info["session_dir"]) / "studyloop"
