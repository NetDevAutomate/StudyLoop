...
class TestBuiltinAdapters:
    ...
    def test_module_exports_adapter(self, name):
        ...
        mod = importlib.import_module(f"studyloop.adapters.{name}")
    ...
    @pytest.mark.parametrize("name", ["codex", "opencode", "pi", "grok"])
...
class TestCodexAdapter:
    def test_codex_setup_writes_agents_md(self, tmp_path):
        from studyloop.adapters.codex import _codex_setup
    ...
    def test_codex_launch_new_session(self):
        from studyloop.adapters.codex import _codex_launch
...
class TestKiroAdapter:
    ...
    def _patch_kiro_dir(self, monkeypatch, tmp_path):
        ...
        import studyloop.adapters.kiro as kiro_mod
    ...
    def test_kiro_setup_creates_persona_file(self, monkeypatch, tmp_path):
        ...
        from studyloop.adapters.kiro import _kiro_setup
    ...
    def test_kiro_setup_backs_up_existing_json(self, monkeypatch, tmp_path):
        ...
        from studyloop.adapters.kiro import _KIRO_BACKUP_SUFFIX, KIRO_AGENT_NAME, _kiro_setup
    ...
    def test_kiro_setup_crash_recovery_restores_stale_backup(self, monkeypatch, tmp_path):
        ...
        # The new target must have the studyloop-managed file:// prompt
        assert target.exists()
        data = json.loads(target.read_text())
    ...
    def test_kiro_teardown_restores_backup(self, monkeypatch, tmp_path):
        ...
        from studyloop.adapters.kiro import _KIRO_BACKUP_SUFFIX, KIRO_AGENT_NAME, _kiro_teardown
    ...
    def test_kiro_teardown_idempotent_no_backup(self, monkeypatch, tmp_path):
        ...
        from studyloop.adapters.kiro import _kiro_teardown
    ...
    def test_kiro_teardown_idempotent_with_target_no_backup(self, monkeypatch, tmp_path):
        ...
        from studyloop.adapters.kiro import KIRO_AGENT_NAME, _kiro_teardown
    ...
    def test_kiro_launch_cmd_no_resume(self, monkeypatch, tmp_path):
        ...
        from studyloop.adapters.kiro import KIRO_AGENT_NAME, _kiro_launch
    ...
    def test_kiro_adapter_teardown_delegates_to_teardown_fn(self, monkeypatch, tmp_path):
        ...
        from studyloop.adapters.kiro import _KIRO_BACKUP_SUFFIX, ADAPTER, KIRO_AGENT_NAME

        kiro_agents.mkdir(parents=True, exist_ok=True)
        target = kiro_agents / f"{KIRO_AGENT_NAME}.json"
        backup = target.with_suffix(target.suffix + _KIRO_BACKUP_SUFFIX)
        backup.write_text(json.dumps({"prompt": "restored"}))
        target.write_text(json.dumps({"prompt": "studyloop-managed"}))
...
class TestLaunchCommands:
    ...
    def test_opencode_launch(self, tmp_path):
        ...
        persona.touch()
        with patch("studyloop.adapters.opencode.shutil.which", return_value="/usr/bin/opencode"):
