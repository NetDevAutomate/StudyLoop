...
- Repair has an energy demand of its own: a live struggle asks for 6/10, an
  ...
  deferred.
- A concept still `learning` is offered as the one-sentence *micro*
  ...
  stuck ladder; the blank is not scored).
...
- A legacy `study_progress` row with no `last_seen` no longer crashes the
  struggle collector.
- The due-review reader emits every `struggling` row as a hands-on "guided
  ...
  earlier fixture had silenced one.
- The Study Session timer's `init()` ran twice per page load, each run
...
- Council records: `docs/architecture/plan-integration/council/` (reviews 1–7
  ...
  a verdict on shipped behaviour.
