...
Codex, OpenCode and Grok Build the session-db MCP server is registered
(Grok Build's own `grok mcp add` CLI does it, not this file); in other
harnesses use `session-query search "<topic>" --project "$PWD"` instead.
