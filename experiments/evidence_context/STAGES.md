...
| 40 — Current intent over older copies | Can archive-only history remain useful while permanent forgetting survives offline cleanup and native replay? | `a63d4859` | `managed_history/runner.py`; five-view standalone-wheel lesson, phase-crash tests, cleanup scale, corrected report-versus-source semantics and council arbitration. Modern full copying/regrant and managed restore remain open. |
