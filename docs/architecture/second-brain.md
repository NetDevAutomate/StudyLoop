...
chosen over two-way synchronisation, and the archived OpenSpec change
`openspec/changes/archive/2026-09-04-second-brain/` for the normative
...
| 2 | **Projections, not sync** | Second Brain backends and `studyloop brain` never mutate source plans. The authorized source-plan writers are `studyloop plan record` and the `record_plan_learning` MCP tool. | `test_second_brain_backend_contract.py::test_operations_never_modify_plan_source`; `test_obsidian_backend.py::test_publish_plan_leaves_source_plan_byte_identical`; `test_plan_record.py::test_first_record_is_lr_0001_and_round_trips`, `test_plan_record.py::test_json_shape` and `test_plan_record.py::test_records_and_reports` |
...
| 6 | **Explicit pull only** | `pull_notes` reads one user-owned sibling file and returns it; it never creates, changes or synchronises anything. Any resulting source-plan write must go through the authorized `studyloop plan record` command or `record_plan_learning` MCP tool. | `test_obsidian_backend.py::test_pull_notes_reads_user_owned_sibling_without_writing` and `::test_pull_notes_missing_sibling_is_found_false_without_error`; `test_cli_brain.py::test_pull_*`; `test_plan_record.py` |
...
| 9 | **Wind-down offers once** | The agent delegates the decision to `studyloop brain wind-down --json`. A configured, publish-capable Obsidian provider gets the pinned publish offer; xTiles gets its pinned offer only when `provider: xtiles` and a connector named exactly `xtiles` are both present. Declining either offer performs no write, and the offer is never repeated. | `test_wind_down_decision.py::test_row_3_obsidian_writable_no_connector`, `test_wind_down_decision.py::test_row_6_xtiles_no_connector`, `test_wind_down_decision.py::test_row_7_xtiles_connector_present` and `test_wind_down_decision.py::test_connector_matching_is_by_exact_name`; `test_wind_down_cli.py::test_cli_matches_the_truth_table_row`; `test_second_brain_docs.py::test_the_wind_down_offer_matches_the_agent_protocol`; `live/test_wind_down_transcripts.py` |
