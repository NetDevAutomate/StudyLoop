...
| 2 | **Projections, not sync** | Second Brain backends and every `studyloop brain` operation read plans and today's data and write only derived notes; they never mutate a source plan. The authorized source-plan writers are `studyloop plan record` and the `record_plan_learning` MCP tool. | `test_second_brain_backend_contract.py::test_operations_never_modify_plan_source`; `test_obsidian_backend.py::test_publish_plan_leaves_source_plan_byte_identical`; `test_plan_record.py::TestRecordLearning`, `::TestPlanRecordCli` and `::TestMcpTool` |
