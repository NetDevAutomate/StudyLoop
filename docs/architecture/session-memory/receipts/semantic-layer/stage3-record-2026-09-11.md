...
## Confirmation seat on the corrections — astra, 2026-09-11 22:35

The corrections were put back to the seat that rejected (brief: the disposition table above, the
full source and test diffs of the correction commit, the addendum receipt keys). Verdict on the
corrections: **REJECT** — one MAJOR, five MINOR. Dispositions, each verified at source:

| id | sev. | finding | disposition |
|---|---|---|---|
| 1 | MAJOR | `candidates()` checked key *existence*; after a rewrite and a re-embed the key exists again while the sidecar still answers with the old text's vector and distance | **CONFIRMED by construction and fixed.** The sidecar `vec0` table now carries `+content_sha256` and `+model` beside each vector; `reconcile()` deletes-and-reinserts any key whose pair differs (counted once in each number) and rebuilds an old-shape sidecar; `candidates()` requires the sidecar pair to equal the canonical row's. Test `test_a_reused_key_does_not_resurrect_the_old_texts_vector`: the raw index answers the old vector at distance ~0, `candidates()` returns nothing, `reconcile()` reports `(1, 1)`, and afterwards the key answers with the new vector's distance. The test **fails when the pair comparison is removed** (checked by neutralising it and restoring) |
| 2 | MINOR | ×4 over-fetch is a heuristic; 40 nearer invalid entries starve 10 live ones | **ACCEPTED**: `candidates()` doubles the raw fetch (up to ×64) while the list is short and the index has more; test with 40 stale-first neighbours and 10 live returns exactly the 10 live |
| 3 | MINOR | ambient-transaction refusal tested, callers not | **ACCEPTED**: real-path test drives `export_sessions._maybe_auto_embed` against a real migrated DB with the model layer stubbed — `in_transaction` is `False` at `embed()` entry and one row is written; `session-maint embed` opens its own connection |
| 4 | MINOR | no-write attribution stronger than the evidence | **ACCEPTED**: the record now says what the receipt shows — the live table still has migration 7's columns and `user_version 47`; the WAL writer is inferred (message count +45, every stage command used `?mode=ro`), not logged |
| 5 | MINOR | cap totals not recomputable from coarse groups; provenance of executed code | **ACCEPTED in part**: the addendum carries the full histogram `h(k)` from which `Σ min(k,cap)·h(k)` and `Σ[k>cap] h(k)` recompute; the corrections receipt names the commit and the tests that passed on the corrected tree |
| 6 | MINOR | equal counts ≠ equal sets | **CLOSED by measurement**: both directional `EXCEPT` queries over eligible message ids on the live DB return **0 rows** (`stage3-corrections-receipt.json`); the historical 13 is attributed to corpus growth as an inference, stated as such |

Measured with the corrections on the clone (`stage3-corrections-receipt.json`): sidecar rebuild
from scratch with the auxiliary columns **174,371 rows in 6.4 s**; no-op reconcile 2.26 s;
`candidates(n=20)` **68 ms** end-to-end on the clone, 20 returned across **8 distinct sessions**
(chunk concentration — Stage 4's fusion aggregates per session).

Stage 3 is **closed on the corrected tree** (the commit that adds this section), with the live
migration and backfill owner-gated as before. The candidate-freshness contract is the one Stage
4 inherits: `candidates()`, never `knn()`.
