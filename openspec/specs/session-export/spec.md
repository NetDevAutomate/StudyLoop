...
Import conversation history from AI coding tools (Claude Code, Codex, Grok,
Kiro CLI, OpenCode and pi) into a shared `sessions.db`, redact secrets on
export paths, and optionally write Obsidian vault notes. This is the
`agent-session-tools` package, distinct from the `studyloop` web/CLI package.
...
`kiro.py`, `opencode.py`, `pi.py` — sharing common persistence helpers in
