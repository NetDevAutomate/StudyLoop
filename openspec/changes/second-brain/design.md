...
| D3 | Package layout `second_brain/{core,factory,obsidian,obsidian_writer,projection,backlinks,templates}`, with providers imported only inside `get_backend` | Makes "`none` imports nothing" provable with a `sys.modules` assertion rather than a code review. |
| D4 | **Withdrawn before release.** The guarded file writer is the only path; nothing spawns a subprocess. The official Obsidian CLI adapter was built and cut in review: it wrote to whichever vault the running app answered for with no way to bind it to `vault_path`, and it passed the rendered plan as an argv element where any local user could read it from the process table. What it bought — the learner's own template firing on a note StudyLoop then overwrote — did not justify either. |
...
| D6 | Atomic temp-file plus `os.replace`; idempotence decided by comparing the rendered projection against the file's own contents, never against the `content_hash` in its marker | Gives idempotence without reading a clock, so a republish is a no-op rather than a churned mtime. The marker's hash records what StudyLoop last *intended* to write, so comparing against it would report a hand-edited projection as unchanged and leave the edit in place — the vault would quietly become a second source of truth. The marker still carries the hash, as evidence of what was written. |
...
| D12 | **Withdrawn with D4.** Nothing writes into a note the learner owns. The daily-note append could not be made at-most-once: the stamp was UTC rather than the learner's calendar day, two concurrent runs both appended, and a non-atomic stamp write read back as "not today". |
