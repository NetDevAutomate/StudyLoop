...
- [x] 5.2 Record redacted connector evidence that an accessible xTiles page URL is HTTPS on exact host `xtiles.app`, has a non-home path, and has no userinfo, port, query, or fragment; verify the strict validator accepts that shape while continuing to reject query and fragment.
